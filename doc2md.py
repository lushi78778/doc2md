#!/usr/bin/env python3
"""
Local DOCX to Markdown converter.

Default behavior:
  - read .docx files from ./original
  - write each result to ./output/<doc-name>/<doc-name>.md
  - write extracted images to ./output/<doc-name>/assets
"""
...
def load_conversion_dependencies():
    try:
        ...
        raise SystemExit(
            "Missing dependency: mammoth or markdownify\n"
            "Recommended setup:\n"
            "  python3 -m venv .venv\n"
...
def sanitize_name(name: str, *, ascii_only: bool = False) -> str:
    """Return a filesystem-friendly name while keeping readable titles by default."""
    name = name.strip()
    if ascii_only:
...
def extension_from_content_type(content_type: str) -> str:
    ...
    return known_extensions.get(content_type, ".png")


def save_mammoth_image(image, assets_dir: Path, image_counter: list[int]) -> dict[str, str]:
    ext = extension_from_content_type(image.content_type)
    image_counter[0] += 1
...
def child_text(element: ET.Element | None) -> str:
    if element is None:
        return ""
    return omml_to_markdown(element)


def wrap_group(text: str) -> str:
    text = text.strip()
    if not text:
        return ""
    if len(text) == 1 or re.match(r"^[A-Za-z0-9τπμΩ]+$", text):
        return text
    return f"{{{text}}}"


def omml_to_markdown(element: ET.Element) -> str:
    tag = element.tag
    ...
    if tag == qname(MATH_NS, "sSub"):
        base = child_text(element.find("m:e", NS))
        sub = child_text(element.find("m:sub", NS))
        return f"{base}_{{{sub}}}"

    if tag == qname(MATH_NS, "sSup"):
        base = child_text(element.find("m:e", NS))
        sup = child_text(element.find("m:sup", NS))
        return f"{base}^{{{sup}}}"
    ...
    if tag == qname(MATH_NS, "f"):
        numerator = child_text(element.find("m:num", NS))
        denominator = child_text(element.find("m:den", NS))
        return f"\\frac{{{numerator}}}{{{denominator}}}"

    if tag == qname(MATH_NS, "rad"):
        degree = child_text(element.find("m:deg", NS))
        expression = child_text(element.find("m:e", NS))
        if degree:
    ...
    if tag == qname(MATH_NS, "nary"):
        operator = child_text(element.find("m:chr", NS)) or "\\sum"
        sub = child_text(element.find("m:sub", NS))
        sup = child_text(element.find("m:sup", NS))
        expression = child_text(element.find("m:e", NS))
        limits = ""
...
def replace_math_in_parent(parent: ET.Element, math_tag: str, display: bool) -> None:
    ...
    for index, child in enumerate(children):
        replace_math_in_parent(child, math_tag, display)
        ...
        replacement = make_text_run(f"{delimiter}{formula}{delimiter}")
        parent.remove(child)
...
def docx_with_markdown_math(docx_path: Path, temp_dir: Path) -> Path:
    ...
    with zipfile.ZipFile(docx_path, "r") as source:
        ...
        replace_math_in_parent(root, qname(MATH_NS, "oMathPara"), display=True)
        replace_math_in_parent(root, qname(MATH_NS, "oMath"), display=False)
        converted_xml = ET.tostring(root, encoding="utf-8", xml_declaration=True)

        with zipfile.ZipFile(converted_path, "w", compression=zipfile.ZIP_DEFLATED) as target:
            for item in source.infolist():
                data = converted_xml if item.filename == "word/document.xml" else source.read(item.filename)
